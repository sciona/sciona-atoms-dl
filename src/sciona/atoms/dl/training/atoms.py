...
@icontract.require(
    lambda size_thresholds, repeat_counts: len(size_thresholds) == len(repeat_counts),
    "size_thresholds and repeat_counts must have the same length",
)
@icontract.require(
    lambda size_thresholds: all(
...
def size_aware_nodule_oversampling(
    ...
    diameter_column: int,
    size_thresholds: NDArray[np.float64] = np.array([6.0, 30.0, 40.0]),
    repeat_counts: NDArray[np.int64] = np.array([1, 2, 4]),
) -> NDArray[np.float64]:
