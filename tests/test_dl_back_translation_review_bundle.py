...
EXPECTED_ATOMS = {"sciona.atoms.dl.back_translation.translate_text"}


def test_back_translation_review_bundle_shape() -> None:
    root = Path(__file__).resolve().parents[1]
    bundle_path = root / "data/review_bundles/dl_back_translation.review_bundle.json"
    bundle = json.loads(bundle_path.read_text())
    assert bundle["provider_repo"] == "sciona-atoms-dl"
    assert bundle["review_status"] == "reviewed"
    assert bundle["family"] == "dl.back_translation"
    rows = bundle["rows"]
    ...
    for row in rows:
        ...
        assert row["review_record_path"] == "data/review_bundles/dl_back_translation.review_bundle.json"
